...
import pandas as pd
from api_betfair import SESSION_TOKEN, callAping, session_token
...
def procurar_jogos_perro_loko():
    ...
    df_events["minuto_aproximado"] = df_events["openDate"].map(
        lambda x: minutos_aproximados(x)
    )  # Quantos minutos aprox o jogo tem
    ...
    logging.info(
        "jogos com mais de %s minutos de horário de início: %s",
        MINUTO_APROXIMADO,
        len(df),
    )
    ...
    if "result" in market_books.keys():
        for mb in market_books["result"]:
            ...
            try:
                df.loc[event_id, "gap"] = (
                    mbr[0]["ex"]["availableToLay"][0]["price"]
                    - mbr[0]["ex"]["availableToBack"][0]["price"]
                )
                df.loc[event_id, "odd_back_under"] = mbr[0]["ex"]["availableToBack"][0][
                    "price"
                ]
    ...
    for index in df.index:
        ...
        try:
            ...
            if event_id not in ignorar_events:
                ignorar_events.append(event_id)
