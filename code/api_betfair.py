...
def callAping(jsonrpc_req: str, endpoint=None) -> dict:
    ...
    headers = {
        "X-Application": APP_KEY,
        "X-Authentication": SESSION_TOKEN[-1],
        ...
        "instructions": [
            {
                ...
                "side": side,
                "orderType": "LIMIT",
                "limitOrder": {"size": stake, "price": odd, "persistenceType": "LAPSE"},
            }
        ],
        "customerRef": f"BZVqFalaNe{selection_id}",
    }

    endPoint = "https://api.betfair.bet.br/rest/v1.0/placeOrders/"

    place_order_response = callAping(
        endpoint=endPoint, jsonrpc_req=str(place_order_req).replace("'", '"')
    )
